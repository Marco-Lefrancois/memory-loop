...
class TestAgentE2ESimulation:
    ...
    def test_boot_sequence_step2_vibe_check(self):
        ...
        if code != 0:
            assert _is_key_alignment_failure(stderr), (
                f"Vibe-check a échoué pour une raison inattendue : {stderr}"
            )
    ...
    def test_boot_sequence_step3_focus_invalid_story_intercepted(self):
        ...
        code, stdout, stderr = run_swarm_cmd(
            ["focus", "--project", PROJECT_NAME, "--story", INVALID_STORY]
        )
        assert code != 0 or "FileNotFoundError" in stdout or "introuvable" in stdout, (
            "L'erreur de story introuvable aurait dû être levée !"
        )
    ...
    def test_boot_sequence_step3_focus_valid_story(self):
        ...
        code, stdout, stderr = run_swarm_cmd(
            ["focus", "--project", PROJECT_NAME, "--story", VALID_STORY]
        )
        assert code == 0, f"Focus valide a échoué avec stderr: {stderr}"
        assert (
            "MMA-4692" in stdout
            or "IN_ANALYZE" in stdout
            or "FOCUS STATE MACHINE" in stdout
            or "Focus" in stdout
        )
    ...
    def test_full_agent_boot_sequence_end_to_end(self):
        ...
        Tolère l'échec clé sur vibe-check (non bloquant pour la chaîne de boot)
        mais exige que resume et focus soient opérationnels.
        """
        # 1. Resume
        c1, out1, _ = run_swarm_cmd(["resume", "--project", PROJECT_NAME])
        assert c1 == 0

        # 2. Vibe Check (tolère key alignment failure)
        c2, out2, err2 = run_swarm_cmd(["vibe-check", "--project", PROJECT_NAME])
        if c2 != 0:
            assert _is_key_alignment_failure(err2), (
                f"Vibe-check a échoué pour une raison inattendue : {err2}"
            )
