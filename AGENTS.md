...
- **Orthogonalité Format × Scope du Protocole Grill-Me (ADR-0393)** :
  - **Axe Format** (`--format`) : `ATOMIC` (1 question/tour) ou `ROUND` (lot de 2-4 questions orthogonales). Choisir `ROUND` ne modifie en rien le périmètre d'analyse et ne constitue EN AUCUN CAS un ordre de rédaction de code ou de story.
  - **Axe Scope** (`--scope`) : `STORY` (récit unitaire via `grill-me`) ou `EPIC/PROJECT` (transverse via `grill-project`).
  - Toujours exécuter `python src/swarm.py grill-project` pour trancher les choix d'architecture transverses (SSO, sécurité, Loi 25, exclusions nettes) avant d'engager les sessions de micro-grilling unitaire `python src/swarm.py grill-me --story <ID>`.
  - **Arrêt Formel Post-Round** : Dès que la frontière de décision d'un round est vide, cessation immédiate de tout appel d'outil d'écriture et présentation du menu d'orientation fermé obligatoire (3 choix : ébauches DRAFT, micro-grill 1:1, clôture).
  - **Mandat Unitaire Strict** : Après un round macro, l'agent ne dispose d'aucun mandat d'écriture par défaut. Seule la sélection explicite d'un récit par l'humain confère le mandat d'instruire ce seul récit (mono-récit strict).
