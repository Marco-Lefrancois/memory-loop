...
def resolve_crawler_cli_path() -> Path:
    ...
    candidates = [
        ...
        Path(__file__).resolve().parent.parent.parent / "mloop-crawler" / "packages" / "cli" / "dist" / "index.js",
    ]
...
def main():
    ...
    if not cli_path.exists():
        sys.stderr.write(f"[MCP CRAWLER] ERREUR FATALE: L'outil mloop-crawler n'est pas compilé ou introuvable à {cli_path}\n")
        sys.stderr.write("[MCP CRAWLER] Exécutez 'npm install && npm run build' dans le dossier mloop-crawler ou définissez MLOOP_CRAWLER_PATH\n")
        sys.stderr.flush()
        sys.exit(1)
