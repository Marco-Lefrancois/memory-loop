    ...
    if architecture_dir.exists() and bus_file.exists():
        ...
        if adrs:
            ...
            if start_marker in content and end_marker in content:
                content = re.sub(rf'{start_marker}.*?{end_marker}', new_adr_block, content, flags=re.DOTALL)
            else:
                content += "\n\n" + new_adr_block
...
def sync_open_questions(project_name: str, project_path: Path) -> None:
    ...
    for md_file in transverse_dir.glob("00-questions-ouvertes-*.md"):
        ...
        for match in re.finditer(r'(?m)^\|\s*((?:Q|QD)-\d+)\s*\|.*?\|\s*([^|]+)\s*\|', content):
            ...
            if any(k in status_text for k in ["décision", "decision", "validé", "valide", "archivé", "archive", "closed"]):
                file_resolved.add(q_id)
            else:
    ...
    if wf_content != original_wf:
        ...
        ZeroFluffConsole.info(f"[Sync] Mise à jour automatique de l'état des OQs dans wayfinder_map.md.")
    ...
    if stories_dir.exists() and resolved_qs:
        for story_file in stories_dir.rglob("*.md"):
            ...
            for q_id in resolved_qs:
                content = re.sub(rf'(?m)^(>.*?⚠️\s*BLOQUÉ.*?\b{q_id}\b.*)$', rf'> ✅ DÉBLOQUÉ : {q_id} (Décision actée)', content, flags=re.IGNORECASE)
...
def sync_sprint_backlog(project_name: str, project_path: Path) -> None:
    ...
    for line in backlog_content.splitlines():
        ...
        if "CODEBASE :" in nline or "MODULE :" in nline or nline.startswith("## ") or nline.startswith("### "):
            current_cat = nline.replace("#", "").replace("MODULE :", "").replace("CODEBASE :", "").strip()
            ...
            if len(parts) >= 3:
                ...
                m_statut = re.search(r'\b(ACCEPTED|DONE|IN_QA|IN_DEV|READY_FOR_DEV|READY_FOR_GROOMING|IN_REVIEW|IN-REVIEW|IN_VALIDATE|IN_PLAN|IN_ANALYZE|OPEN|ON_HOLD|ON-HOLD|CLOSED|BACKLOG)\b', row_text, re.IGNORECASE)
    ...
    for f in stories_dir.rglob("*.md"):
        ...
        target_status = direct_id_status_map.get(story_id.upper()) or status_map.get((category, story_id.upper())) or direct_id_status_map.get(f.stem.upper())
        if target_status:
    ...
    if updated_files > 0:
        ZeroFluffConsole.info(f"[Sync] {updated_files} fichier(s) de récit(s) réaligné(s) sur le statut de sprint_backlog.md (SSOT).")
            ...
            try:
                ...
                if proc.returncode == 0:
                    ...
                    if "Already up to date" in out or "Déjà à jour" in out:
                        ...
                        ZeroFluffConsole.success(f"[Live Git-Sync] {wiki_p.name} synchronisé avec succès : {out.splitlines()[0]}")
                else:
                    ZeroFluffConsole.warning(f"[Live Git-Sync] Avertissement pull sur {wiki_p.name} : {proc.stderr.strip()[:100]}")
            except Exception as e:
                ZeroFluffConsole.warning(f"[Live Git-Sync] Synchronisation live ignorée pour {wiki_p.name} : {e}")

def sync_hypergraph(project_name: str, project_path: Path, incremental: bool = False, verbose: bool = False) -> None:
    """
    ...
    try:
        ...
        if story_dir.exists():
            for story_file in story_dir.rglob("*.md"):
                try:
                    ...
                    business_rules = [m.group(1) for m in re.finditer(r'\b(BR-\d{3}|RM-\d{3})\b', fc)]
                    apis = [m.group(1) for m in re.finditer(r'\b(API-\d{3}|GET\s+/[^\s]+|POST\s+/[^\s]+|PUT\s+/[^\s]+|DELETE\s+/[^\s]+)\b', fc)]
                    data_models = [m.group(1) for m in re.finditer(r'\b(MDL-\d{3})\b', fc)]
                    gherkin_tests = [m.group(1).strip() for m in re.finditer(r'(?i)^\s*(?:Scénario|Scenario):\s*(.+)$', fc, re.MULTILINE)]
                    ...
                    ka.create_story_unit(
                        ...
                        business_rules=sorted(list(set(business_rules))) if business_rules else None,
                        data_models=sorted(list(set(data_models))) if data_models else None,
                        adrs=sorted(list(set(adrs))) if adrs else None,
                        gherkin_scenarios=sorted(list(set(gherkin_tests))) if gherkin_tests else None,
                        status=status
                    )
                except Exception:
                    pass
        ...
        if existing_ka and incremental:
            ...
            if verbose or report.has_changes:
                ZeroFluffConsole.info(f"[Hypergraph Sync] Fusion incrémentale : {len(report.added_nodes)} nœuds ajoutés, {len(report.updated_nodes)} mis à jour, {len(report.added_edges)} arêtes ajoutées.")
        else:
            ka.save_to_file(hypergraph_file)
            if verbose or len(ka.edges) > 0:
                ZeroFluffConsole.info(f"[Hypergraph Sync] Hypergraphe généré ({len(ka.nodes)} nœuds, {len(ka.edges)} hyper-arêtes) -> {hypergraph_file.name}")
...
def run_sync(project_name: str, state: LoopState, project_path: Path, details_msg: str = None, fast_mode: bool = False, verbose: bool = False, incremental: bool = False) -> LoopState:
        ...
        for sf in story_files:
            ...
            for v in report.violations:
                if v.severity == "BLOCKING":
                    ZeroFluffConsole.warning(f"[struct-check] {sf.name} : [{v.check_id}] {v.message}")
                    struct_violations_found = True
        if struct_violations_found:
            ZeroFluffConsole.warning("[struct-check] Violations structurelles détectées. Exécuter 'python src/swarm.py struct-check --verbose' pour le détail.")
    ...
    try:
        ...
        if verbose or not fast_mode:
            ZeroFluffConsole.info(f"[Fact-Search FTS5] {chunks_count} chunks documentaires et {lex_count} termes du lexique indexés.")
