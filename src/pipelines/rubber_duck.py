...
class RubberDuckEngine:
    ...
    def evaluate_file(self, target_file: Path, force: bool = False) -> Dict[str, Any]:
        ...
        critique_result = {
            ...
            "suggestions": suggestions or ["Considérer la journalisation des événements d'audit (X-Correlation-ID)."],
        ...
        self.trace_logger.log_trace(
            ...
            prompt_context={"file": str(target_file), "rules": ["Gherkin-4P", "INVEST", "MermaidGate"]},
            tool_calls=[{"tool": "rubber_duck_read_only_eval", "status": "success"}],
            validation_result={"wikifix_passed": not bool(blocking_issues), "error_code": "ERR_RUBBER_DUCK" if blocking_issues else "NONE"},
    ...
    def save_review_report(self, critique_result: Dict[str, Any]) -> Path:
        ...
        category = target_p.parent.name if target_p.parent.name in ["FOOD", "COMMERCE", "SANTE"] else None
        ...
        status_badge = "🟢 CONFORME (Prêt pour Dev)" if is_approved else "🔴 AJUSTEMENTS REQUIS (Problèmes fonctionnels bloquants)"
        ...
        if critique_result["blocking_issues"]:
            ...
            lines.append("- ✅ **Validation** : Les règles énoncées sont logiquement cohérentes et conformes aux exigences du domaine.")
