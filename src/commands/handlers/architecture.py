...
def handle_grill(args: argparse.Namespace, state: LoopState, project_path: Path) -> int:
    ...
    search_term = getattr(args, "query", None) or getattr(args, "title", None) or getattr(args, "story", None)
    ...
    adr_path = ge.record_adr(
        ...
        context=getattr(args, "context", "Contexte issu d'une session de grilling interactive."),
        decision=getattr(args, "decision", "Décision arbitrée conjointement."),
        positives=getattr(args, "positives", "Clarification du domaine et réduction de l'ambiguïté."),
    ...
    if getattr(args, "story", None):
        if ge.mark_story_grilled(args.story):
            ZeroFluffConsole.success(f"Récit {args.story} marqué comme GRILLED (statut: READY_FOR_GROOMING).")
        else:
            ZeroFluffConsole.warning(f"Récit {args.story} introuvable dans backlog/stories ou sprint_backlog.md.")
    run_sync(args.project, state, project_path)
    return 0


def handle_wayfinder(args: argparse.Namespace, state: LoopState, project_path: Path) -> int:
...
def handle_to_spec(args: argparse.Namespace, state: LoopState, project_path: Path) -> int:
...
def handle_to_tickets(args: argparse.Namespace, state: LoopState, project_path: Path) -> int:
    ...
    res = tp.decompose_to_tickets(
        ...
        [{"title": "Initialisation du module principal", "type": "BE", "blocked_by": []}],
    )
    ZeroFluffConsole.success(f"Stories générées dans backlog/stories ({len(res)} récits).")
    run_sync(args.project, state, project_path)
    return 0


def handle_graph_run(args: argparse.Namespace, state: LoopState, project_path: Path) -> int:
    ...
    ZeroFluffConsole.info(f"Lancement de l'exécution Graph Engineering (DAG Multi-Agents) pour : {args.project}")
    ...
    if wf.map_file.exists():
        ...
        router = GraphRouter(initiative_name=initiative_title, project_dir=str(project_path))
        n1 = TaskNode(id="node_spec", role="orchestrator", description="Spécification des besoins & Rôles", blocked_by=[], category=NodeCategory.LLM_AGENT, risk_level=RiskLevel.LOW)
        n2 = TaskNode(id="node_plan_be", role="plan", description="Plan d'architecture Back-End & Modèle", blocked_by=["node_spec"], category=NodeCategory.LLM_AGENT, risk_level=RiskLevel.HIGH)
        n3 = TaskNode(id="node_plan_fe", role="plan", description="Plan d'architecture Front-End & UI", blocked_by=["node_spec"], category=NodeCategory.LLM_AGENT, risk_level=RiskLevel.MEDIUM)
        n4 = TaskNode(id="node_reducer", role="reducer", description="Nœud Déterministe : Fusion & Déduplication des EvidencePacks (Barrier/Reduce)", blocked_by=["node_plan_be", "node_plan_fe"], category=NodeCategory.DETERMINISTIC_CODE, risk_level=RiskLevel.LOW)
    ...
    if getattr(args, "resume", False):
        if router.load_checkpoint():
            ZeroFluffConsole.success("Checkpoint d'exécution chargé. Les nœuds déjà complétés seront sautés.")

    # Exécution orchestrée du DAG avec transport des EvidencePacks
    if router.nodes.get("node_spec") and router.nodes["node_spec"].status != "completed":
        if router.mark_running("node_spec"):
            pack_spec = EvidencePack(source_node="node_spec", initiative_name=initiative_title)
            pack_spec.add_item(EvidenceItem(
                evidence_id="SPEC-01",
        ...
        if router.nodes.get(nid) and router.nodes[nid].status != "completed":
            if router.mark_running(nid):
                ...
                pack.add_item(EvidenceItem(
                    ...
                    evidence_payload={"route": "/api/v1/onetrust/consent", "method": "POST", "fields": ["consent_id", "status"]}
                ))
    ...
    if router.nodes.get("node_reducer") and router.nodes["node_reducer"].status != "completed":
        if router.mark_running("node_reducer"):
            merged_pack = EvidenceReducer.merge_and_deduplicate(packs_for_reducer, target_node_name="node_reducer")
            router.mark_completed("node_reducer", evidence_pack=merged_pack)

    summary = router.get_summary()
    ZeroFluffConsole.success(f"DAG Execution Summary: {summary['completed']}/{summary['total_nodes']} complété(s).")
    ZeroFluffConsole.info("Événements Herdr diffusés vers memory/herdr_events.jsonl.")
    return 0 if router.is_completed() else 1


def handle_deepen(args: argparse.Namespace, state: LoopState, project_path: Path) -> int:
    ...
    ZeroFluffConsole.success(f"Rapport HTML de profondeur d'architecture généré : {report_path}")
    return 0


def handle_diagnose(args: argparse.Namespace, state: LoopState, project_path: Path) -> int:
    ...
    ZeroFluffConsole.success(f"Harnais de reproduction déterministe généré : {harness_path}")
    return 0


def handle_goal_cascade(args: argparse.Namespace, state: LoopState, project_path: Path) -> int:
    ...
    ZeroFluffConsole.success(f"Goal-Cascading : {report['aligned_stories']}/{report['total_stories']} récit(s) alignés (Score: {report['alignment_score']}%).")
    return 0


def handle_to_sow(args: argparse.Namespace, state: LoopState, project_path: Path) -> int:
