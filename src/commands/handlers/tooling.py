...
def handle_svg_optimize(args: argparse.Namespace, state: LoopState, project_path: Path) -> int:
    ...
    if not input_val:
        ...
        if p.is_absolute():
            ...
            pattern = input_val
        matched_files = (
            list(search_dir.glob(pattern))
            if search_dir.exists()
            else list(project_path.glob(input_val))
        )
        ZeroFluffConsole.info(
            f"Optimisation de {len(matched_files)} fichier(s) correspondant au motif : {input_val}"
        )
        res = engine.optimize_files(matched_files)
    else:
        target_path = (
            Path(input_val) if Path(input_val).is_absolute() else (project_path / input_val)
        )
        ZeroFluffConsole.info(f"Optimisation des fichiers SVG sous : {target_path}")
...
def handle_hill_climb(args: argparse.Namespace, state: LoopState, project_path: Path) -> int:
    ...
    if new_score > initial_score:
        ZeroFluffConsole.success(
            f"Amélioration validée : {new_score:.2f} > {initial_score:.2f}. Mutation conservée."
        )
    else:
        ZeroFluffConsole.warning(
            f"Régression ou statut quo : {new_score:.2f} <= {initial_score:.2f}. Rollback."
        )
...
def handle_archify(
    ...
    if not input_file:
        LoggingConsole.error(
            "L'argument --file <chemin_json> est obligatoire (ou utilisez --doctor).",
            command="archify",
    ...
    if not input_path.exists():
        LoggingConsole.error(
            f"Fichier de spécification introuvable : {input_path}",
            command="archify",
            project=getattr(args, "project", None),
            target_keys=str(input_path),
        )
        return 1
    ...
    if args.validate_only:
        ZeroFluffConsole.info(
            f"Validation Showcase de la spécification Archify [{d_type}] : {input_path}"
        )
    ...
    ZeroFluffConsole.info(
        f"Compilation Archify Showcase [{d_type}] : {input_path} -> {output_path}"
    )
    code = run_archify_command(
        "deliver", d_type, str(input_path), output_path=str(output_path), quality=args.quality
    )
    if code == 0:
...
def handle_drawdb(
    ...
    ZeroFluffConsole.info(
        f"Démarrage du visualiseur relationnel de base de données sur le port {port}..."
    )
    serve_drawdb(port=port, auto_open=auto_open)
    return 0


# Handlers CSV extraits dans tooling_csv.py (decoupage ADR-0202) et reexportes
# ci-dessous pour preserver le routing '_registry.py' (tooling:handle_csv_*).
from src.commands.handlers.tooling_csv import (  # noqa: E402,F401
    handle_csv_normalize,
    handle_csv_validate,
    handle_csv_anonymize,
    handle_csv_diff,
)
