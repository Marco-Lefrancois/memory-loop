...
def validate_gate4_approval(project_path: Path, approver: str) -> Dict[str, Any]:
    ...
    if not qa_report_file.exists() and (Path("Projects") / proj_name / "memory" / "qa_certification_report.json").exists():
    ...
    try:
        ...
        raise ValueError(f"Approbation de Gate 4 refusée : rapport QA corrompu ou illisible : {exc}")
    ...
    if not approver_clean or (is_bot_only and not has_human):
        raise ValueError(
            ...
            "humaine formelle (ex: approver='Marco' ou 'Lead Architect')."
        )
