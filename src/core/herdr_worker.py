...
class HerdrWorkerMixin:
    ...
    def spawn_story_worker(
        ...
        try:
            ...
            logger.debug(f"wait_agent timeout pour '{worker_name}' — prompt envoyé quand même.")
        clean_target = (
            str(story_id)
            .replace("\\", "/")
            .replace("Projects/", "")
    ...
    def cleanup_worker(self, story_id_or_pane: str) -> Dict[str, Any]:
        ...
        if ":" not in pane_id:
            ...
            if agents_res.get("success"):
                ...
                for ag in self._extract_agents_list(agents_res.get("result", {})):
    ...
    def audit_and_reap_zombies(self, project_name: Optional[str] = None) -> Dict[str, Any]:
        ...
        reaped = []
        for ag in self._extract_agents_list(list_res.get("result", {})):
            ...
            if (
                ...
                logger.info(f"Reaping zombie '{name}' in '{pane_id}' (status: {status})...")
