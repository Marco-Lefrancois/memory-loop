...
class ContextGuard:
    ...
    def truncate_tool_output(
        output_text: str, max_lines: int = 70, head_lines: int = 50, tail_lines: int = 20
    ) -> str:
        """
    ...
    INJECTION_PATTERNS = [
        ...
        re.compile(
            r"(?i)\byou\s+are\s+now\s+(?:an?\s+unrestricted|in\s+developer\s+mode|dan|a\s+new\s+agent)\b"
        ),
        ...
        for pattern in cls.INJECTION_PATTERNS:
            ...
            if matches:
                ...
                sanitized = pattern.sub(
                    "[CONSIGNE_NEUTRALISÉE: TENTATIVE_INJECTION_DÉTECTÉE]", sanitized
                )
    ...
    def offload_heavy_tool_output(
        ...
        try:
            ...
            logger.debug(
                "Écriture du sidecar de sortie volumineuse échouée",
                exc_info=True,
                extra={"sidecar_dir": str(sidecar_dir), "hash": content_hash, "error": str(e)},
            )
