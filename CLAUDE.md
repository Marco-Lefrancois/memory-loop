...
**Matrice des Synchronisations mLoop** :
...
- **Principe Dual-Track (Skill in-process vs Worker out-of-process - ADR-0346)** :
  - **Piste A (Skill In-Process)** : Directives méthodologiques chargées dans la mémoire de travail de l'orchestrateur (`view_file` sur `SKILL.md`) pour savoir *comment* formater et arbitrer sans créer de processus.
  - **Piste B (Worker Out-of-Process)** : Spawning obligatoire d'une session vierge Herdr (`worker-spawn`, `worker-handoff-test`, `worker-legacy-mine`, `worker-shadow-estimate`, `worker-visual-dissect`, `worker-janitor-watch`) dès qu'une tâche nécessite de l'isolation, du calcul lourd ou une impartialité contradictoire.
- **Délégation aux Sous-Sessions (Fork & Harvest Herdr — Déclenchement Obligatoire)** : La délégation via `python src/swarm.py worker-spawn` est **obligatoire** (non optionnelle) dès qu'au moins **un** des critères suivants est rempli :
  ...
  - 🔬 **Type de tâche (5 Gates Stratégiques)** :
...
> 3. Type = Red Team / Spike / Ingestion / Consensus / Test ? → `worker-spawn`
...
- **Gating Constitutionnel des Équipes & Sous-Agents (ADR-0362)** :
  - **Sous-Agents Hiérarchiques (Défaut)** : Privilégier l'isolation stricte de contexte pour les tâches ciblées de recherche, analyse ou codage. Lors de l'envoi de directives volumineuses, privilégier l'injection par fichier de prompt éphémère (`--append-subagent-system-prompt-file` ou fichier scratch) plutôt que l'argument CLI brut.
  - **Agent Teams (Phase 2 Création Uniquement)** : Réservé exclusivement à la conception contradictoire et à l'exploration ToT multi-perspectives. Tout déploiement d'Agent Teams doit obligatoirement être borné (`max_turns <= 5`) pour juguler le facteur multiplicateur de tokens (3x-7x).
...
- ✅ **Always do (Toujours Faire)** :
  ...
  - **Standard Grill-with-Docs & Dossier de Preuves Documentaires (Passage-Level Grounding Inconditionnel - ADR-0320 / ADR-0326 / ADR-0361)** : Lors de tout cadrage, analyse ou incubation de récit, obligation formelle de restituer IN EXTENSO dans le chat (et d'enregistrer sous `memory/evidence/<STORY_ID>_fact_dossier.md`) le **Dossier de Preuves Documentaires & Faits Établis** complet (1. Maquettes SSOT & Notes d'Atelier avec liens `file:///...`, 2. Matrice de résolution des conflits, 3. Extraits verbatim sourcés avec numéros de lignes précis et faits établis déduits `Extrait N — Titre (Lignes X-Y) : « Citation » ➔ Fait établi : ...`, 4. Structure de données DBML / Mermaid ERD, 5. Contrats déclaratifs cibles REST/CTA, 6. Évaluation de la frontière active, 7. Admission of Limits). **Interdiction formelle de remplacer ce dossier par un plan d'intention abstrait, un méta-plan ou une liste à puces vide de substance.**
  ...
  - **Gouvernance Multi-Modèles LiteLLM** : Sélection dynamique (`claude-opus-4.8`, `gpt-5.6-terra-thinking`, `claude-sonnet-5`, `claude-sonnet-4.6`, `gemini-3.8-flash`, `gemini-3.7-flash`, `gpt-transcribe`) via `opencode`, `pi`, `omp`, `agy` ou `worker-spawn`.
  - Philosophie *Wu Wei* et auto-apprentissage continu via `python src/swarm.py sync`.
  - **Plafonnement de Mémoire 200 Lignes (Règle des 200 Lignes / 25 Ko - ADR-0362)** : Le fichier racine `memory/SESSION_MEMORY_HEALTH.md` est strictement plafonné à 200 lignes et 25 Ko. Les historiques et détails de preuves volumineux sont déportés dans `memory/evidence/summaries/consolidated_evidence_index.json`.
  - **Confinement Local des Diagrammes (Zéro Exfiltration)** : Rendu de diagrammes 100% local (Mermaid, SVG, Graphviz). Interdiction formelle d'exfiltrer du code ou des données vers des serveurs de rendu distants non souverains.
...
- 🚫 **Never do (Ne Jamais Faire)** :
  ...
  - **Violation du Contrat Visuel (Maquettes = SSOT)** : Ne jamais contredire une maquette. Si une maquette UI/UX existe (`docs/05-assets/` ou `reference/maquettes_svg/`), **elle est la source de vérité absolue pour l'interface** (y compris ses déclinaisons verticales/mobiles). En cas de conflit entre un compte-rendu textuel et la maquette validée, la maquette l'emporte. L'Agent doit strictement décrire le composant dessiné. **Maquette Vectorisée = Lecture Obligatoire** : si un SVG ingéré ne contient aucune balise `<text>`/`<tspan>` (texte vectorisé/outlined), le pipeline d'ingestion invoque automatiquement le pont OCR (`src/converters/svg_ocr_bridge.py`, skill `.agents/skills/svg-ocr/`) pour en extraire le contenu réel ; le statut de lecture (`is_vectorized`/`ocr_status`) est tracé dans le Markdown ingéré et dans le bloc `visual_contract` de l'EvidencePack (`memory/evidence/<STORY_ID>_evidence.json`), et audité par le 10ᵉ contrôle Vibe-Check « Contrat Visuel Lisible ».
  ...
  - **Zéro Troncature ni Code Paresseux (Full-Output Enforcement - output-skill)** : Interdiction formelle d'émettre des commentaires d'omission (`// TODO`, `/* rest of code */`, `...`) ou de livrer des implémentations partielles. Chaque fichier généré ou édité doit être complet, exhaustif et directement exécutable/compilable. En front-end, interdiction formelle du "AI-Slop" (dégradés violets génériques, cartes bento symétriques monotones, héros 100vh sans hiérarchie) conformément au skill `.agents/skills/design-taste/SKILL.md`.
  - **Zéro Création de Compétence Jetable (Zero-Bloat Skills - ADR-0362)** : Interdiction d'ajouter des compétences unitaires dans `.agents/skills/` sans audit préalable via `python src/swarm.py doctor --skills`. Bonifier les compétences maîtresses existantes (`calibrate`, `dream_consolidator`, `rho_optimizer`) pour préserver le budget de boot sous le seuil de 15 000 jetons.
