---
name: handoff
description: "Handoff: Condense le contexte et clôt la session."
...
## 🧱 Core Principles
- **Smart Zone vs Dumb Zone** : Un agent est beaucoup plus intelligent au début de sa session (Smart Zone). Ce skill permet de conserver la continuité du travail tout en redémarrant avec une conscience fraîche.
...
3. **Contenu Requis** :
   ...
   - **Pointeurs** : Liens Markdown vers les fichiers pertinents et les artefacts créés (ex: `docs/adr/`, `task.md`, etc.).
...
Lorsque l'analyse d'une story est achevée (`READY_FOR_DEV`) et prête à être transmise au développement (développeur humain ou agent IA comme Renaud, Cursor, Copilot, etc.) :
