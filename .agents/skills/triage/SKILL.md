...
4. **Découpage Vertical Pur (Vertical Slicing) & Macrostructures UI (ADR-0340)** :
   ...
   - Pour chaque récit `layer: frontend` ou `layer: fullstack`, sélectionner une **macrostructure UI dédiée** parmi les 21 formes du catalogue (`standards/blueprints/ui_macrostructures.md`) et appliquer la **règle de diversification** (zéro répétition de structure entre écrans consécutifs).
...
Si le document d'entrée est incomplet ou ambigu, consigner explicitement les zones d'ombre sous forme de questions ouvertes (OQ-XXX) sans bloquer le découpage des parties claires.
